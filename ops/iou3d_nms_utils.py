...
from ops.utils import check_numpy_to_torch
import cuda_ops 
import numpy as np
...
def boxes_bev_iou_cpu(boxes_a, boxes_b):
    ...
    cuda_ops.boxes_iou_bev_cpu(boxes_a.contiguous(), boxes_b.contiguous(), ans_iou)
...
def boxes_iou_bev(boxes_a, boxes_b):
    ...
    cuda_ops.boxes_iou_bev_gpu(boxes_a.contiguous(), boxes_b.contiguous(), ans_iou)
...
def nms_gpu(boxes, scores, thresh, pre_maxsize=None, **kwargs):
    ...
    num_out = cuda_ops.nms_gpu(boxes, keep, thresh)
...
def nms_normal_gpu(boxes, scores, thresh, **kwargs):
    ...
    keep = torch.LongTensor(boxes.size(0))
    num_out = cuda_ops.nms_normal_gpu(boxes, keep, thresh)
